...
#define NUM_PIEZOS 5 // Number of Pads/Cymbals
#define NUM_FOOT_SWITCHES 1
#define START_SLOT A0     //first analog slot of piezos
#define DIGITAL_START_SLOT 7 // first digital slot for pedals
...
#define HTOM_THRESHOLD 200
#define MTOM_THRESHOLD 200
#define LTOM_THRESHOLD 200
// ---------------------------
#define HIHAT_THRESHOLD 200  //update these once we know how cymbals are gonna work
...
// for outputting to Rock Band Brain
const byte NUM_RB_OUTPUTS = 4;
// list of RB pins
// outputs in same order as inputs (START_SLOT input maps to [0] in this array)
byte rbOutPins[NUM_RB_OUTPUTS] = {5, 6, 9, 10};
...
void setup() {
  ...
  if(RB_OUT){
    //for(short i=0; i<NUM_RB_OUTPUTS; i++){
    //  pinMode(rbOutPins[i], OUTPUT);
    //}
  }
  ...
  //Bass note needs to be pulled up
  pinMode(footSwitchSlotMap[0], INPUT_PULLUP);
  pinMode(footSwitchNoteMap[0], OUTPUT);
...
void loop() {
  ...
  for (short i=0; i<NUM_FOOT_SWITCHES; ++i) {
    ...
    if ( ( dSignal == LOW  ) && (footSwitchLastNote[i] != LOW) ) {
        ...
        if(RB_OUT) digitalWrite(footSwitchNoteMap[i], LOW);
        ...
        if(RB_OUT) digitalWrite(footSwitchNoteMap[i], HIGH);
    }
